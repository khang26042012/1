...
// OneMinApiProfile — shows the connected 1min.ai (API key) account profile.
// Mirrors OneMinProfile but hits the API-key-scoped /onemin-api-profile endpoint.
export default function OneMinApiProfile({ connectionId }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/providers/${connectionId}/onemin-api-profile`, { cache: "no-store" });
        if (res.ok) {
    ...
    <div className="flex items-center gap-3 rounded-lg border border-border-subtle bg-surface-2/50 px-3 py-2">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <p className="truncate text-sm font-medium text-text-main">1min.ai (API)</p>
          {profile.plan && (
