...
// ── PAT (Personal Access Token) support — port of VansRouter ──────────────
// PATs (pt-...) cannot sign COSY requests directly; they are exchanged for
// short-lived job tokens (jt-...) which then sign like device tokens. The
// exchange result is cached + deduplicated (concurrent first-time callers
// share one upstream request per PAT).
...
/**
 * Resolve PAT-based credentials into device-token-shaped credentials the COSY
 * signing path can use. Non-PAT credentials pass through unchanged.
 */
